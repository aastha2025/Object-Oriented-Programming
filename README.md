...
3types 
**Public** :-> All class members can be accessible to everyone.
          :->  by using direct dot (.)can be accessed.
    
**Private**:->  By default the access modifier for the members will be Private.
...
## constructors: 
 Constructor  name is the same as the class name .Constructor is invoked at the time of object creation.It do not return value, hence they do not have a return type. It gets called automatically when we create the object of the class.  Provides data for the object. can be overloaded.can not be declared virtual.
There are total 4 types of constructor:
**copy  , parametrized , default , move**

## this :
this is the  pointer to  current object.
here it contain same words on both the side like 
this->h=h;
a special pointer that allows objects to refer to themselves within their scope.
